...
class Login(QtWidgets.QDialog):
    ...
    def handleLogin(self):
        ...
        client = MongoClient(
            "mongodb+srv://CalcUser:CalcUser@cluster0.xyexc.mongodb.net/<dbname>?retryWrites=true&w=majority")
...
def recalculate():
    ...
    if window.UVSystem.currentText() == 'R-200':
        ...
        window.RED.setText(
            str(round(config.LampFactor * RED(config.Drive, config.Drive, config.LampEfficiency, config.LampEfficiency
                                              , config.FlowRate_m3h / config.nBranches, config.UVT, config.D1Log,
                                              config.NLamps), 1)))
        if (float(window.RED.toPlainText()) < 0):
            window.RED.setText('0')
        window.RED.setAlignment(QtCore.Qt.AlignCenter)
        window.HeadLoss.setText(
            str(round(config.HL_Multiplier * HL(config.FlowRate_m3h / config.nBranches, config.NLamps), 2)))
    ...
    if window.UVSystem.currentText() == 'RZ-163-UHP':
        ...
        # Calculate RED and HeadLoss
        window.RED.setText(str(round(
            config.LampFactor * RED(config.Drive, config.LampEfficiency, config.FlowRate_m3h / config.nBranches,
    ...
    if window.UVSystem.currentText() == 'WW module':
        ...
        if config.NLamps == 11:
            window.HeadLoss.setText(
                str(round(config.HL_Multiplier * WW11.HeadLoss(config.FlowRate_m3h / config.nBranches), 2)))
        elif config.NLamps == 6:
            window.HeadLoss.setText(
                str(round(config.HL_Multiplier * WW6.HeadLoss(config.FlowRate_m3h / config.nBranches), 2)))
    ...
    if window.UVSystem.currentText() == 'RZ-104':
        ...
        if (float(window.RED.toPlainText()) < 0):
            window.RED.setText('0')
        window.RED.setAlignment(QtCore.Qt.AlignCenter)
        window.HeadLoss.setText(str(round(config.HL_Multiplier * HL(config.FlowRate_m3h / config.nBranches,
                                                                    lampsPerBranch[window.UVModel.currentText()]), 2)))
        window.HeadLoss.setAlignment(QtCore.Qt.AlignCenter)
    ...
    if window.UVSystem.currentText() == 'RZ-163':
        ...
        window.RED.setText(str(round(
            config.LampFactor * RED(config.Drive, config.Drive, config.Drive, config.Drive, config.LampEfficiency,
                                    config.LampEfficiency,
    ...
    if window.UVSystem.currentText() == 'RS-104':
        ...
        # Calculate RED and HeadLoss
        window.RED.setText(str(round(
            lampsPerBranch[window.UVModel.currentText()] * config.LampFactor * RED(config.Drive, config.LampEfficiency,
                                                                                   config.FlowRate_m3h / config.nBranches,
                                                                                   config.UVT, config.D1Log,
                                                                                   config.NLamps), 1)))
        if (float(window.RED.toPlainText()) < 0):
            window.RED.setText('0')
        window.RED.setAlignment(QtCore.Qt.AlignCenter)
    ...
    if window.UVSystem.currentText() == 'RZB-300':
        ...
        # Calculate RED and HeadLoss
        window.RED.setText(str(round(
            config.LampFactor * RED(config.Drive, config.Drive, config.LampEfficiency, config.LampEfficiency,
                                    config.FlowRate_m3h / config.nBranches,
                                    config.UVT, config.D1Log, config.NLamps), 1)))
        if (float(window.RED.toPlainText()) < 0):
            window.RED.setText('0')
    ...
    if window.UVSystem.currentText() == 'RZM-200':
        ...
        # Calculate RED and HeadLoss
        window.RED.setText(
            str(round(config.LampFactor * RED(config.Drive, config.Drive, config.Drive, config.Drive, config.Drive,
    ...
    if (window.UVSystem.currentText() == 'RZM-350') | (window.UVSystem.currentText() == 'RZM-350-Marine'):
        ...
        # Calculate RED and HeadLoss
        window.RED.setText(str(round(
            VF * config.LampFactor * RED(config.Drive, config.Drive, config.Drive, config.Drive, config.Drive,
                                         config.Drive, config.Drive, config.Drive,
                                         config.LampEfficiency, config.LampEfficiency, config.LampEfficiency,
                                         config.LampEfficiency,
                                         config.LampEfficiency, config.LampEfficiency, config.LampEfficiency,
                                         config.LampEfficiency,
    ...
    if (
            float(window.HeadLoss.toPlainText()) > config.HeadLossWarning * config.HL_Multiplier):  # Set warning on HeadLoss >2m
...
def PlotREDvsUVT():
    ...
    def mouseMoved(evt):
        ...
        if p1.sceneBoundingRect().contains(pos):
            ...
            if mousePoint.y() < 0:
                label.setText(
                    "<span style='color: black'>UVT=%0.1f[%%-1cm],   <span style='color: red'>RED=%0.1f[mJ/cm²]</span>" % (
                    mousePoint.x(), 0))
            else:
                label.setText(
                    "<span style='color: black'>UVT=%0.1f[%%-1cm],   <span style='color: red'>RED=%0.1f[mJ/cm²]</span>" % (
...
def PlotREDvsFlow():
    ...
    def mouseMoved(evt):
        ...
        if p1.sceneBoundingRect().contains(pos):
            ...
            if mousePoint.y() < 0:
                label.setText(
                    "<span style='color: black'>Flow=%i[m³/h],   <span style='color: red'>RED=%0.1f[mJ/cm²]</span>" % (
                    mousePoint.x(), 0))
            else:
                label.setText(
                    "<span style='color: black'>Flow=%i[m³/h],   <span style='color: red'>RED=%0.1f[mJ/cm²]</span>" % (
...
def PlotREDvsDrive():
    ...
    def mouseMoved(evt):
        ...
        if p1.sceneBoundingRect().contains(pos):
            ...
            if mousePoint.y() < 0:
                label.setText(
                    "<span style='color: black'>Power=%i[%%],   <span style='color: red'>RED=%0.1f[mJ/cm²]</span>" % (
                    mousePoint.x(), 0))
            else:
                label.setText(
                    "<span style='color: black'>Power=%i[%%],   <span style='color: red'>RED=%0.1f[mJ/cm²]</span>" % (
...
def DechloCalc():  # Recalculate Dechlorination and ozone decomposition
    ...
    try:
        ...
        config.ChlorineOut = round(
            (config.ChlorineIn) * (0.5) ** (float(window.RED.toPlainText()) / (config.ChlorineD_05)), 2)
...
def UVModel():
    try:  # Module modifiers in list
        NLamps = config.Lamp_Modules.get(window.UVModel.currentText()) * config.lamps_in_single_Module.get(
            window.UVSystem.currentText())
    ...
    params = config.SystemParameters.loc[(config.SystemParameters['UV-System'] == window.UVSystem.currentText()) & (
                config.SystemParameters['Model'] == window.UVModel.currentText())]
...
def LampEfficiency():
    ...
    try:
        ...
        if (float(window.LampEfficiency.text()) >= config.minEfficiency) & (
                float(window.LampEfficiency.text()) <= config.maxEfficiency):
...
def FlowRate():
    ...
    try:
        ...
        if ((config.FlowUnits == 'm3h') & (float(window.FlowRate.text()) >= (config.minFlow)) & (
                float(window.FlowRate.text()) <= (config.maxFlow))):
            ...
            ValidInput = True
            recalculate()
        elif ((config.FlowUnits == 'gpm') & (float(window.FlowRate.text()) >= (config.minFlowgpm)) & (
                float(window.FlowRate.text()) <= (config.maxFlowgpm))):  # USgpm units
...
def FlowForDose():
    ...
    TargetDose, okPressed = QtWidgets.QInputDialog.getDouble(window, "Calculate Flow", "Dose Value:",
                                                             float(window.RED.toPlainText()), 0, 10000, 2)
    if okPressed:
